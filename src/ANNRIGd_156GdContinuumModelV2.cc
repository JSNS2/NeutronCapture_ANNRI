...
{
	std::cout << "ANNRIGd_156GdContinuumModelV2 : Initializing model..." << std::endl;
	...
	if(not inDataFileName.empty()) {
		...
		if(inFile and not inFile->IsZombie()) {
			...
			std::cerr << "ANNRIGd_156GdContinuumModelV2 : ERROR! Could not open "
					"input data file <"
					<< inDataFileName
					<< "> - ABORTING!" << std::endl;
			abort();
		...
		std::cerr << "ANNRIGd_156GdContinuumModelV2 : ERROR! Given input data file "
				"name is empty - ABORTING!" << std::endl;
		abort();
	}

	std::cout << "ANNRIGd_156GdContinuumModelV2 : Done!" << std::endl;
}
